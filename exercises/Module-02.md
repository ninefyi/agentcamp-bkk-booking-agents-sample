...
│  ┌──────────────────┐         ┌─────────────────┐              │
│  │ Convert query    │────────▶│ Search MongoDB  │              │
│  │ to embedding     │         │ Atlas for       │              │
│  │                  │         │ similar listings│              │
│  └──────────────────┘         └────────┬────────┘              │
│                                         │                        │
│                                         ▼                        │
...
│  6. Generation (LLM)                                             │
│  ┌──────────────────┐                                           │
│  │ Azure OpenAI GPT │──────▶  Conversational response           │
│  └──────────────────┘                                           │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
- **`generate_embedding(text)`** — converts text to a 1536-dimension vector using Azure OpenAI
- **`vector_search(query, limit, filters)`** — runs a `$vectorSearch` aggregation pipeline against MongoDB Atlas
...
The `get_llm()` function creates an Azure OpenAI client, and `format_listings_simple()` helper are already implemented for you.
